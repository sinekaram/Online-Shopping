...
try{
	...
	ResultSet rs1=st.executeQuery("select sum(product_total) from cart where email='"+email+"'");
	while(rs1.next()){
		total=rs1.getInt(1);
	}
%>
        ...
        <tbody>
        <%
     	ResultSet rs=st.executeQuery("select * from product inner join cart on product.id=cart.id and cart.email='"+email+"'");
     	while(rs.next()){
     	%> 
          <tr>
          <%sno=sno+1; %>
...
<div class="right-div">
<h3>Pay online on this online_shopping@pay.com</h3>
